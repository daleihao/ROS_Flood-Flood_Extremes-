...
for region_i = 1:4
    ...
    for delta_T = 0:5
        ...
        SOILWATERs =  SOILICEs_init + SOILLIQs_init;
        SOILWATERs(~masks) = nan;
        ...
        if(delta_T ==0)
            ...
            plot_global_map(region_i, lats, lons, SOILWATERs, 0, limits(region_i), 'Control', 1, 1,'');
            colormap(ax1, colors_abs);
            ylabel(event_names{region_i});
            SOILWATER_hist = SOILWATERs;
            if region_i==1
            ...
            diff_tmp = (SOILWATERs - SOILWATER_hist);
...
set(gcf, 'Color', 'w');
exportgraphics(gcf,'../../figure_P/Figure_S_soil_moisture_spatial_map_r_P.pdf','Resolution',300);
close all
