function [Variable_elevs, meanelevation] = get_elev_gradient_variable_1500(region_i, variable_name)
...
for delta_T = 0:5
    ...
    for elevation_i = 1:2
        ...
        if elevation_i == 1
        filters = meanelevation <= 1500;
        else
        filters = meanelevation> 1500;
        end
        Variable_elevs(elevation_i,delta_T+1 ) = mean(variable_spatial(filters),'omitnan');
