function plot_subfigure_elevation_gradient_runoff(region_i, variableName,islegend,isylabel,xlabeltext,ylabeltext,indexlabel,ymin,ymax,xmin,xmax,event_name)
...
dif_var = variable_elevs(:,6) - variable_elevs(:,1);
rel_var = dif_var(:)./variable_elevs(:,1)*100/5;
...
if islegend
legend([p1 p2 p3 p4 p5 p6],{'Control', '+1K', '+2K', '+3K', '+4K', '+5K'},'location','east')
end
xlabel(xlabeltext);
if isylabel
ylabel(ylabeltext);
...
text(xmax-(xmax-xmin)/20,YL(2) - (YL(2) - YL(1))*0.05, indexlabel,'fontsize',18,'FontWeight','bold');
if region_i <3
text(xmax-(xmax-xmin)/3.5,YL(1) + (YL(2) - YL(1))*0.05, event_name,'fontsize',16,'FontWeight','bold');
else
text(xmax-(xmax-xmin)/3,YL(1) + (YL(2) - YL(1))*0.05, event_name,'fontsize',16,'FontWeight','bold');
end
box on
